...
class ManipulateEnv(hand_env.HandEnv):
    ...
    def _goal_distance(self, goal_a, goal_b):
        ...
        if self.target_rotation != 'ignore':
            ...
            quat_c = np.copy(quat_b)
            quat_c[-1] = -quat_b[-1]
            quat_diff = rotations.quat_mul(quat_a, rotations.quat_conjugate(quat_b))
            angle_diff = 2 * np.arccos(np.clip(quat_diff[..., 0], -1., 1.))
            quat_diff_2 = rotations.quat_mul(quat_a, rotations.quat_conjugate(quat_c))
            angle_diff_2 = 2 * np.arccos(np.clip(quat_diff_2[..., 0], -1., 1.))
            d_rot = min(angle_diff, angle_diff_2)

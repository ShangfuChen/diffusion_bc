...
class Diff_bc(BaseILAlgo):
    ...
    def init(self, policy, envs, args):
        ...
        if not args.eval_only:
            ...
            num_step = 100
            if self.args.action_only:
                self.model = MLPDiffusion(100, input_dim = 2).to(self.args.device)
            else:
    ...
    def _bc_step(self, decay_lr):
        ...
        pred_loss, expert_loss = self.get_density(self.model, states, pred_actions, true_actions, self.args.action_only)
        pred_density, expert_density = math.exp(-pred_loss), math.exp(-expert_loss)
        if self.args.agent_loss_only:
            diff_loss = pred_loss
        else:
        ...
        if self.args.diff_coefficient_scheduler:
            ...
            coefficient = self.args.coefficient * 2.5 * max(0,(x-0.6))
        ...
        with torch.no_grad():
            ...
            for batch in self.val_train_loader:
                ...
                if self.args.diff_coefficient_scheduler:
                    x = self.num_epochs / self.args.bc_num_epochs
                    coefficient = self.args.coefficient * 2.5 * max(0,(x-0.6))
                    total_loss = action_loss + coefficient*diff_loss
                elif self.args.diff_coefficient_scheduler_2:
                    x = self.num_epochs / self.args.bc_num_epochs
                    coefficient = self.args.coefficient * 1.25 * max(0,(x-0.2))
                else:
                    total_loss = action_loss + self.args.coefficient*diff_loss

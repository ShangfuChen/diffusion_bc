...
class MLPDiffusion_next(nn.Module):
    def __init__(self, n_steps, input_dim=256, num_units=128, device='cuda'):
        super(MLPDiffusion_next,self).__init__()
...
if __name__ == '__main__':
    ...
    dataset = torch.cat((obs[:, :4], next_obs, actions), 1)
    ...
    model = MLPDiffusion_next(num_steps, input_dim=dataset.shape[1], num_units=128).to(device)
    model.load_state_dict(torch.load('maze2d_100_nextobs_deeper_ddpm_deep_12000.pt'))
    optimizer = torch.optim.Adam(model.parameters(),lr=4*1e-5)
    #scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor = 0.8, patience=2, verbose = True)

    train_loss_list = []
    for t in tqdm(range(12000, num_epoch)):
        total_loss = 0
        ...
        if t % 20 == 0:
            ...
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.title('nextobs_ddpm_deeper_loss.png')
            plt.savefig(env + '_nextobs_ddpm_deeper_loss_{}.png'.format(num_epoch))
            plt.close()
            ...
            with torch.no_grad():
                ...
                file_name = env + '-nextobs-deeper-ddpm-reconstruct-{}.png'.format(num_epoch)
                plt.savefig('results/' + file_name)
                plt.close()    
    
    torch.save(model.state_dict(), env + '_nextobs_deeper_ddpm_deep_{}.pt'.format(num_epoch))

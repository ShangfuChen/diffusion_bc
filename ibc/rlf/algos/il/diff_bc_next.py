...
from rlf.algos.il.density.ddpm_nextbos import MLPDiffusion_next
import math

class Diff_bc_next(BaseILAlgo):
    """
    ...
    def init(self, policy, envs, args):
        ...
        num_step = 100
        self.model = MLPDiffusion(100, input_dim = 8).to(self.args.device)
        weight_path = self.args.ddpm_path
        self.model.load_state_dict(torch.load(weight_path))
        
        self.model_next = MLPDiffusion_next(100, input_dim = 12).to(self.args.device)
        weight_path = self.args.ddpm_next_path
        self.model_next.load_state_dict(torch.load(weight_path))
    ...
    def get_density(self, model, states, pred_action, expert_action, action_only=False):
        num_steps = 100
    ...
    def get_density_next(self, model, states, pred_next, pred_action, expert_next, expert_action):
        num_steps = 100
        # decide beta
        betas = torch.linspace(-6,6,num_steps)
        betas = torch.sigmoid(betas)*(0.5e-2 - 1e-5)+1e-5
        ...
        pred = torch.cat((states[:,:4], pred_next, pred_action), 1)
        expert = torch.cat((states[:,:4], expert_next, expert_action), 1)
        pred_loss, expert_loss = self.diffusion_loss_fn(model, pred, expert, alphas_bar_sqrt, one_minus_alphas_bar_sqrt, num_steps)
    ...
    def _bc_step(self, decay_lr, t=1): # t means the process of training
        if decay_lr:
            super().pre_update(self.num_bc_updates)
        ...
        if t > self.args.next_sche:
            pred_loss, expert_loss = self.get_density_next(self.model_next, states, pred_next_states, pred_actions, true_next_states, true_actions)
            pred_density, expert_density = math.exp(-pred_loss), math.exp(-expert_loss)
            diff_loss = torch.clip(pred_loss - expert_loss, min=0)

            #pred_loss, expert_loss = self.get_density(self.model, states ,pred_actions, true_actions)
            #pred_density, expert_density = math.exp(-pred_loss), math.exp(-expert_loss)
            #diff_loss_2 = torch.clip(pred_loss - expert_loss, min=0)

            #total_loss = loss + self.args.coefficient/2 * diff_loss + self.args.coefficient/2 * diff_loss_2
            total_loss = loss + self.args.coefficient * diff_loss
            ...
            if self.args.diff_coefficient_scheduler:
                x = self.num_epochs / self.args.bc_num_epochs
                coefficient = self.args.coefficient * 2 * max(0,(x-0.5))
                total_loss = loss + coefficient*diff_loss

            elif self.args.diff_coefficient_scheduler_2:
                x = self.num_epochs / self.args.bc_num_epochs
                coefficient = self.args.coefficient * 1.25 * max(0,(x-0.2))
                total_loss = loss + coefficient*diff_loss
            else:
                total_loss = loss + self.args.coefficient * diff_loss
    ...
    def update(self, storage, t=1):
        ...
        log_vals = self._bc_step(True, t) #_pr_action_loss
    ...
    def get_add_args(self, parser):
        ...
        parser.add_argument("--diff-coefficient-scheduler", type=bool, default=False)
        parser.add_argument("--diff-coefficient-scheduler-2", type=bool, default=False)
        parser.add_argument("--test-diff", type=bool, default=False)
        parser.add_argument("--agent-loss-only", type=bool, default=False)
        parser.add_argument("--action-only", type=bool, default=False)
        parser.add_argument("--ddpm-path", type=str, default=None)
        parser.add_argument("--ddpm-next-path", type=str, default=None)
        parser.add_argument("--next-sche", type=float, default=0.7)

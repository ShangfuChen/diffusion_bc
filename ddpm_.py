...
if __name__ == '__main__':
    ...
    #print("done", data['done'].shape) #[18525]
    #print("obs", data['obs'][0]) #[18525, 6]
    #print("next_obs", data['next_obs'][:, :4].shape) #[18525, 6]
    #print("actions", data['actions'].shape) [18525, 2]
    ...
    trian_dataset = dataset[:14820]
    val_dataset = dataset[14820:]
    train_loader = torch.utils.data.DataLoader(trian_dataset,batch_size=batch_size,shuffle=True)
    ...
    for ep in tqdm(range(1,num_epoch+1)):
        ...
        for idx, batch_x in enumerate(train_loader):
            ...
            optimizer.step()
        ave_loss = sum(epoch_loss) / len(train_loader)
        ...
        with torch.no_grad():
            epoch_loss = []
            for idx, batch_x in enumerate(val_loader):
                batch_x = batch_x.squeeze()
                loss = diffusion_loss_fn(model,batch_x,alphas_bar_sqrt,one_minus_alphas_bar_sqrt,num_steps)
                epoch_loss.append(loss.cpu().detach().item())
            ave_loss = sum(epoch_loss) / len(val_loader)
            val_loss_list.append(ave_loss)
            if(ep % 100 == 0):
                print(ep, " epoch ", "val_loss:", ":", ave_loss)
        ...
        if ep % 20 == 0:
            ...
            plt.savefig(env + '_model_loss_split.png')
            plt.close()

    torch.save(model.state_dict(), env + '_ddpm_nextobs_.pt')
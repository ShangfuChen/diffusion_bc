...
class MLPDiffusion(nn.Module):
    def __init__(self, n_steps, input_dim=6, num_units=2048, device='cuda'):
    ...
    for t in tqdm(range(0, num_epoch)):
        ...
        if t % 100 == 0:
            ...
            with torch.no_grad():
                ...
                axs[0].scatter(dataset[:300, 2], dataset[:300, 5], color='blue', edgecolor='white')
                axs[1].scatter(out[:300, 2], out[:300, 5], color='red', edgecolor='white')
                plt.savefig(f'{env}-foot.png')
                plt.close()
        if t % 20 == 0:

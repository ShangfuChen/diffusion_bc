...
from dm.ddpm import MLPDiffusion
import os
...
class Diff_gail(NestedAlgo):
    def __init__(self, agent_updater=PPO(algo_name='GAIL'), get_discrim=None):
...
class GailDiscrim(BaseIRLAlgo):
    ...
    def get_loss(self, agent_state, expert_state, agent_action, expert_action):
        num_steps = 100
        dim = agent_state.size()[1] + agent_action.size()[1]
    ...
    def _update_reward_func(self, storage):
        ...
        for epoch_i in range(self.args.n_gail_epochs):
            for expert_batch, agent_batch in zip(expert_sampler, agent_sampler):
                ...
                dim = expert_batch['state'].size()[1] + expert_batch['actions'].size()[1]
                num_steps = 100
                model = MLPDiffusion(num_steps, input_dim = dim).to(self.args.device)
                diff_loss = 0.01 * self.get_loss(agent_batch['state'], expert_batch['state'], agent_batch['action'], expert_batch['actions'])
                ...
                log_vals['diff_loss'] += diff_loss.item()

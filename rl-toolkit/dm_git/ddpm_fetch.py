...
class MLPDiffusion(nn.Module):
    def __init__(self, n_steps, input_dim=6, num_units=1024, device='cuda'):
...
if __name__ == '__main__':
    ...
    model = MLPDiffusion(num_steps,
                         input_dim=dataset.shape[1]).to(device)
    ...
    for t in tqdm(range(0, num_epoch)):
        ...
        if t % 100 == 0:
            ...
            with torch.no_grad():
                ...
                axs[0].scatter(dataset[:300, 14], dataset[:300, 15], color='blue', edgecolor='white')
                axs[1].scatter(out[:300, 14], out[:300, 15], color='red', edgecolor='white')
                plt.savefig(f'{env}-goal.png')
                plt.close()
                ''' 
                fig, axs = plt.subplots(1, 2, figsize=(28, 3))
                axs[0].scatter(dataset[:300, -1], dataset[:300, -2], color='blue', edgecolor='white')
                axs[1].scatter(out[:300, -1], out[:300, -2], color='red', edgecolor='white')
                plt.savefig(f'{env}-action1.png')
                plt.close()
                fig, axs = plt.subplots(1, 2, figsize=(28, 3))
                axs[0].scatter(dataset[:300, -2], dataset[:300, -3], color='blue', edgecolor='white')
                axs[1].scatter(out[:300, -2], out[:300, -3], color='red', edgecolor='white')
                plt.savefig(f'{env}-action2.png')
                plt.close()
                '''
        if t % 20 == 0:
            ...
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.title(env + '_loss.png')
            plt.savefig(env + '_loss.png')
            plt.close()    
        if t % 1000 == 0:
